...
{
    public class LoginCommands : BaseCommands
    {
        public LoginCommands(UserSessionManager sessionManager) : base(sessionManager)
        {
            _commands = new Dictionary<string, Func<long, ITelegramBotClient, Update, Task>>
            {
                { "/menu", ViewLoginMenuMessageCommandAsync },
                { "/start", ViewLoginMenuMessageCommandAsync },
